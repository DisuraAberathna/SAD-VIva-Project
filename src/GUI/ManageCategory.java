...
public class ManageCategory extends javax.swing.JDialog {
    ...
    public ManageCategory(java.awt.Frame parent, boolean modal, Product product) {
        super(parent, modal);
        initComponents();
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (name.isEmpty()) {
            ...
            try {
                ResultSet resultSet = Database.execute("SELECT * FROM `category` WHERE `name` = '" + name + "'");
                if (resultSet.next()) {
                    JOptionPane.showMessageDialog(this, "This category already exsits", "Warning", JOptionPane.WARNING_MESSAGE);
                } else {
                    Database.execute("INSERT INTO `category` (`name`, `status`) VALUES ('" + name + "', '" + status + "')");
                    JOptionPane.showMessageDialog(this, "Category successfully added", "Information", JOptionPane.INFORMATION_MESSAGE);
                    clear();
                    loadCategory();
                }
            } catch (SQLException ex) {
        ...
        if (String.valueOf(jTable1.getValueAt(row, 2)).equals("Active")) {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                ...
                if (name.isEmpty()) {
                    ...
                    try {
                        ResultSet resultSet = Database.execute("SELECT * FROM `category` WHERE `name` = '" + name + "' AND NOT `id` = '" + id + "'");
                        if (resultSet.next()) {
                            JOptionPane.showMessageDialog(this, "This category already exsits", "Warning", JOptionPane.WARNING_MESSAGE);
                        } else {
                            Database.execute("UPDATE `category` SET `name` = '" + name + "',  `status` = '" + status + "' WHERE `id` = '" + this.id + "'");
                            JOptionPane.showMessageDialog(this, "Category successfully updateed", "Information", JOptionPane.INFORMATION_MESSAGE);
                            clear();
                            loadCategory();
                        }
                    } catch (SQLException ex) {
            ...
            JOptionPane.showMessageDialog(this, "Not a active category", "Warining", JOptionPane.WARNING_MESSAGE);
        }
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        if (row != -1) {
            if (String.valueOf(jTable1.getValueAt(row, 2)).equals("Active")) {
                this.product.setCategoryId(String.valueOf(jTable1.getValueAt(row, 0)));
                this.product.getjTextField3().setText(String.valueOf(jTable1.getValueAt(row, 1)));
                this.dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Please select a active category", "Warning", JOptionPane.WARNING_MESSAGE);
            }
        }
